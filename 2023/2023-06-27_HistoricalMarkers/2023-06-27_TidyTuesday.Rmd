...
plotFinal <- ggplot() +
  geom_point(data = placeNames %>% drop_na(prim_long_dec), aes(x = prim_long_dec, y = prim_lat_dec), shape = 20,
             colour = "#AC1D1C", size = 0.1, alpha = 0.2) +
  geom_map(data = usMap, map = usMap, aes(map_id = region, group = group), fill = NA, colour = "#074E67") +
  labs(title = "Historical Markers in the United States",
       subtitle = glue::glue("Geographical distribution of historical markers in the conterminous United States. The top three states with the most historical markers are {placeNames_grouped[1,1]} with {placeNames_grouped[1,2]}, {placeNames_grouped[2,1]} with {placeNames_grouped[2,2]}, and {placeNames_grouped[3,1]} with {placeNames_grouped[3,2]}."),
       caption = "Source: Historical Marker Database USA Index | #TidyTuesday | Week 26") +
  theme_void() +
