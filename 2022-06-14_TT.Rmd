...
drought <- drought %>%
  mutate(Perc = ifelse(grepl('D', DI) == Perc, Perc, Perc * -1), 
         DI = factor(DI), 
...
colPal <- c("black", "black", "black", "blue", "red",
            "black", "black", "black", "black", "black") 


plotDrought <- ggplot(drought) +
  geom_text(aes(as.Date('2010-10-15'), y = 65, label = state_abb), 
            size = 4, color = 'grey70', family = 'sans', fontface = 'bold') +
  geom_area(aes(x = DATE, y = Perc, fill = labels)) +
  ...
  theme(plot.background = element_rect(fill = 'aliceblue', colour = 'aliceblue'),
        ...
        legend.position = 'top',
        legend.title = element_text(family = 'sans', face = 'bold', size = 45, hjust = 0.5, color = cols[7]),
        legend.text = element_text(family = 'sans', face = 'bold', color = 'grey60', size = 17, vjust = 5),
        plot.margin = margin(6.5, 1, 1.5, 1, unit = 'cm')) +
  guides(fill = guide_legend(nrow = 2, byrow = TRUE, reverse = TRUE, title = 'Conditions',
                             title.position = 'top', label.position = 'bottom',
                             keywidth = 8, vjust = -5))

Annotated <- ggdraw(plotDrought) +
  draw_text(x=0.5, y=0.95, text="Drought & Wet Conditions", family= "sans",
            fontface="bold", size=88, color= cols[6]) +
  draw_text(x=0.5, y=0.86, 
            text="Plot displays abnormal drought and wet conditions in the United States from 2012 to 2022. Area plot\nrepresents the total land area of each state experiencing the specified condition.", 
            family= "sans", fontface="bold", size=25, color=cols[7], lineheight=1) +
  draw_text('Source: National Integrated Drought Information System', x=0.5, y=0.02, 
            fontface="bold", size=16, family = "sans", color=cols[6])
